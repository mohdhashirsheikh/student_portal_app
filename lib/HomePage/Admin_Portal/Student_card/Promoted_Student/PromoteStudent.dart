...
class _PromoteStudentState extends State<PromoteStudent> {
  ...
  Future<void> promoteStudent() async {
    if (promoteSession == null &&
      ...
      var response = await http.post(
          Uri.parse("http://sniic.co.in/admin/school_app/Promote_Student.php"),
          body: json.encode(data));
      var obj = jsonDecode(response.body);
      if (obj["result"] == "S") {
        setState(() {
          errorPromote = "Students Promoted Successfully";
          isLoadingPromote = false;
          srnum = "";
          countToValue = {};
        });
        ...
        setState(() {
          isLoadingPromote = false;
          errorPromote = "Student are not Promote some error";
        });
      }
    }
  }
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return Container(
                      ...
                      child: CheckboxListTile(
                        ...
                        subtitle: Text(searchStudentList![index]["regno"]),
                        selected:
